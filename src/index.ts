...
async function openBrowser(endpoint: string, options: Options, configPage?: (page: playwright.Page, browser: playwright.Browser) => Promise<void>): Promise<playwright.BrowserContext | undefined> {
	...
	const browser = await browserType.launch({ headless, args, devtools: options.devTools, env: {'DMD': 1}, executablePath: "/Applications/Firefox Nightly.app/Contents/MacOS/firefox", firefoxUserPrefs: {"DMD": 1} });
	console.log(browser);
